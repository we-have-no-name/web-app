    ...
    def write(self, data):
        ...
        try:
            ...
            if 'retweeted_status' in tweet_json:
                tweet = "RT @" + tweet_json['retweeted_status']['user']['screen_name'] + ": " + \
                        tweet_json['retweeted_status']['text']
            else:
...
class TwitterAgent():
    ...
    def get_sample_tweets_stream(self, **kwargs):
        ...
        if kwargs:
            raise TypeError("'{}' is an invalid keyword argument for this function".format(next(iter(kwargs))))
        stream = self.make_stream_object(file_name, lang=lang, add_timestamp=add_timestamp, max_tweets=max_tweets,
                                         save_to_files=save_to_files, data_handler=data_handler, data_list=data_list)
        stream.sample()
        return data_list
